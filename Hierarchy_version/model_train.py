...
from get_exact_match_window1 import get_exact_match_window_data, get_exact_match_window_testdata
from lib.torch_utils import non_neg_normalize, np_softmax
from lib.data_utils import list_shuffle, pad_batch_list, pad_batch_dlist
...
def load_params(config_path):
    ...
    param_dict = {"batch_size": batch_size, "vocab_size": vocab_size, "emb_size": emb_size, 
                  "hidden_size": hidden_size, "dropout": dropout, "preemb": preemb, 'window_size': window_size,
                  "hinge_margin": hinge_margin, "n_epoch": n_epoch, "alpha": alpha,  
...
def train(config_path, resume=True):
    ...
    model = Attention(emb_size=emb_size, query_length=q_len, doc_length=d_len, num_heads=num_heads, 
        ...
        dropout=dropout, qrep_dim=output_dim, hidden_size=hidden_size, batch_size=batch_size,
    ...
    for epoch in range(1, n_epoch + 1):
        ...
        for f in train_files:
            data = load_dataset(f)
            data = get_exact_match_window_data(data, int(window_size))
            ...
            valid_data = get_exact_match_window_testdata(valid_data, int(window_size))
            ''' shuffle data'''
            ...
            for batch_count in range(num_batch):
                ...
                if use_cuda:
                    ...
                    D_pos = Variable(torch.LongTensor(pad_batch_dlist(D_pos, max_len=50, padding_id=0)), requires_grad=False).cuda()
                    D_neg = Variable(torch.LongTensor(pad_batch_dlist(D_neg, max_len=50, padding_id=0)), requires_grad=False).cuda()
                    ...
                    D_pos = Variable(torch.LongTensor(pad_batch_dlist(D_pos, max_len=d_len, padding_id=0)), requires_grad=False)
                    D_neg = Variable(torch.LongTensor(pad_batch_dlist(D_neg, max_len=d_len, padding_id=0)), requires_grad=False)
                    L = Variable(torch.FloatTensor(L), requires_grad=False)
                
                # run on this batch
                optimizer.zero_grad()
                t1 = time.time()
                D_pos = D_pos.view(batch_size, D_pos.size(1)*D_pos.size(2))
                D_neg = D_neg.view(batch_size, D_neg.size(1)*D_neg.size(2))
                #assert D_pos.size() == (128, 1100)
...
def predict(config_path, model, data, mode="valid"): #pass the predict data
    ...
    for topic_num in data:
        ...
        if use_cuda:
            ...
            D_test = Variable(torch.LongTensor(pad_batch_dlist(D, max_len=50, padding_id=0)), requires_grad=False).cuda()
        else:
            Q_test = Variable(torch.LongTensor(pad_batch_list(Q, max_len=q_len, padding_id=0)), requires_grad=False)
            D_test = Variable(torch.LongTensor(pad_batch_dlist(D, max_len=d_len, padding_id=0)), requires_grad=False)
        
        scores = []
        for batch_id in range(num_batch):
            Q_value = Q_test[batch_id * batch_size: (batch_id + 1)* batch_size]
            D_value = D_test[batch_id * batch_size: (batch_id + 1)* batch_size]
            D_value = D_value.view(D_value.size(0), D_value.size(1)*D_value.size(2))
            Q_mask, D_mask, _ = model.generate_mask(Q_value, D_value, D_value)
            batch_rel, _ = model(Q_value, D_value, D_value, Q_mask, D_mask, D_mask)  
            if use_cuda:
...
def test(config_path):
    ...
    test_data = get_exact_match_window_testdata(test_data, int(window_size))
    q_len = param_dict['q_len']
    d_len = param_dict['d_len']
    emb_size = param_dict['emb_size']
    num_heads = param_dict['num_heads']
