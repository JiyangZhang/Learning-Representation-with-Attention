...
class MultiHeadAttention(nn.Module):
  ...
  def __init__(self, word_dim, query_length, doc_length, num_heads, output_dim
    ...
    self.output_linear = nn.Linear(word_dim, output_dim, bias=False)
  ...
  def forward(self, queries, keys, values):
    ...
    keys = self.linear_trans(keys)
    values = self.linear_trans(values)
    
    # Split into multiple heads
    queries = self._split_heads(queries) # SHAPE=[batch_size, num_heads, seq_length, dim/num_heads]
    ...
    logits = torch.matmul(queries, keys.permute(0, 1, 3, 2))  #SHAPE=[bs, num_heads, query_length, doc_length]
    ...
    contexts = torch.matmul(weights, values)  #SHAPE=[bs, num_heads, query_length, dim/num_heads]
